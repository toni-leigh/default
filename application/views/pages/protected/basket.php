            ...
            // voucher is output too, based on what is set here
                        ...
                        <span class='birow_details bipostrow_details'>Postage:</span>
                            <select id='pclass' name='pclass' onchange='update_basket_postage()'>
                                <?php foreach ($pclasses as $pclass): ?>
                                    ...
                                    <option value='<?=$pclass['pclass_name']; ?>' <?=$selected; ?>><?=str_replace('_',' ',$pclass['pclass_heading']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        <span class='birow_subtotal bipostrow_subtotal'>&pound;<span id='postage_total'><?=number_format($b['subtotal'],2); ?></span></span>
                        <?php $postage_total=$b['subtotal']; ?>
                        <input type='hidden' id='current_postage' value='<?=$postage_total; ?>'/>
                ...
                else
                {
                    ...
                    // now output the row itself, the voucher row too
                        ?>
                        <div class='birow <?=$vrow_class; ?>'>
                            <span class='birow_details'>
                                <span class='birow_qty'>
                                    <input id='<?=$b['rowid']; ?>' class='basket_quantity form_field rounded' name='<?=$b['rowid']; ?>' type='text' value='<?=$b['qty']; ?>' onkeyup='check_numeric(\"#<?=$b['rowid']; ?>\")'/>
                                    <span class='birow_qtyx'>X</span>
                                </span>
                                <span class='birow_name'><?=$b['name']; ?></span>
                                <span class='birow_price'>@ <?=format_price($b['price']); ?> each.</span>
                                <span class='birow_subtotal'><?=format_price($b['subtotal'],2); ?></span>
...
<div id='content_right'>
    ...
    <div id='nv_total_row' class='total_row'>
        ...
        <span id='nv_total_val' class='gt_val'>
            ...
            <div id='nv_grand_total'>
                <?=number_format($total['total_nv'],2); ?>
            </div>
        </span>
    </div>
    ...
    <div id='voucher_apply'>
        <?=form_open('/voucher/check_voucher'); ?>
